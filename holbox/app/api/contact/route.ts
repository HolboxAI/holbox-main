...
export async function POST(req: NextRequest) {
  ...
  const smtpUser = process.env.SES_SMTP_USER;  // SES SMTP username
  const smtpPass = process.env.SES_SMTP_PASS;  // SES SMTP password
  const salesEmail = 'sales@holbox.ai';  // Verified "From" email address

  if (!smtpUser || !smtpPass) {
    return NextResponse.json({ error: "SMTP configuration is missing" }, { status: 500 });
  }

  // Create a transporter using SES SMTP
  const transporter = nodemailer.createTransport({
    host: 'email-smtp.us-east-1.amazonaws.com',  // SES SMTP endpoint (replace with your region)
    port: 587, // SMTP port for SES
    secure: false, // Use TLS (starttls)
    auth: {
      user: smtpUser,  // SES SMTP username
      pass: smtpPass,  // SES SMTP password
    },
  });

  // HTML content for company email
  const companyEmailHTML = `
    <html lang="en">
    <head>
      <meta charset="UTF-8">
      ...
      <style>
        ...
        .container { max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; padding: 20px; box-shadow: 0 2px 6px rgba(0, 0, 0, 0.1); }
        .header { background-color: #4a90e2; color: #ffffff; padding: 20px; text-align: center; border-radius: 8px 8px 0 0; }
        .content { padding: 20px; }
        .content p { line-height: 1.6; font-size: 16px; color: #333; margin-bottom: 16px; }
        .footer { text-align: center; font-size: 12px; color: #777; background-color: #f9fafb; padding: 15px; border-radius: 0 0 8px 8px; }
      </style>
      <title>New Contact Form Submission</title>
    </head>
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <p><strong>Subject:</strong> Contacted by ${subject} from contact form at holbox.ai</p>
          <p><strong>Message:</strong><br/>${message}</p>
        </div>
        <div class="footer">
          <p>This message was sent from your contact form on your website.</p>
  ...
  // HTML content for user confirmation email
  const userConfirmationHTML = `
    <html lang="en">
    <head>
      <meta charset="UTF-8">
  ...
  try {
    ...
    await transporter.sendMail({
      from: salesEmail,  // "sales@holbox.ai" as the "From" email
      to: salesEmail,  // Same email as the recipient (sales@holbox.ai)
      replyTo: email,
      subject: `New Contact Form Submission: ${subject}`,
      html: companyEmailHTML,
    });
    ...
    await transporter.sendMail({
      from: salesEmail,  // Same "From" address for the user confirmation
